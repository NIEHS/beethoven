...
target_baselearner <-
  list(
    ...
    targets::tar_target(
      ...
      command = list(
        mlp = expand.grid(
          hidden_units = c(256, 512),
          dropout = c(0.25, 0.3),
          activation = c("relu"),
          learn_rate = c(0.001, 0.0005, 0.0001),
          epochs = c(500, 750, 1000)
        ),
        elnet = expand.grid(
          mixture = seq(0, 1, length.out = 4),
          penalty = 10 ^ seq(-1, 1, 1)
        ),
        lgb = expand.grid(
          mtry = floor(
            c(0.025, seq(0.05, 0.2, 0.05)) * ncol(dt_feat_calc_xyt_subset)
          ),
    ...
    targets::tar_target(
      ...
      command = list(
        dt_full = dt_feat_calc_xyt_subset,
        r_subsample = 0.3,
        folds = NULL,
        tune_mode = "grid",
        tune_grid_size = 12L,
        yvar = "Arithmetic.Mean",
        xvar = seq(5, ncol(dt_feat_calc_xyt_subset)),
        nthreads = 2L,
        trim_resamples = TRUE,
        return_best = TRUE,
        cv_rep = 100L
...
target_baselearner_gpu <-
  list(
    ...
    targets::tar_target(
      ...
      command = c(fit_learner_base_cpu),
