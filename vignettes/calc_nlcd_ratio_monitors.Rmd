...
nlcd_small <- terra::crop(nlcd_2021, ext, mask=T) 
plot(nlcd_2021) 
plot(ext, add=T, col="black") 
plot(nlcd_small)
...
## Compute NLCD ratio in 150m buffers

```{r}
start <- Sys.time()
aqs_mon_nlcd <- calc_nlcd_ratio(data_vect = aqs_mon, 
                                buf_radius = 150, 
                                year = 2021)
end <- Sys.time()
end - start
...
ggplot() + 
  ...
  geom_scatterpie(aes(x=long, y=lat, r = .5), data=df,
