...
# source functions
home <- "../"
source(paste0(home, "/R/download.R"))
source(paste0(home, "/R/download_support.R"))
source(paste0(home, "/R/download_test_support.R"))
```
...
functions_sources <- data.frame(Function, Source)
...
For an alternate example, we can use a start and end date that are known to not have data. As the URLs associated with these dates do not exist, we expect `all(url_status)` to return `FALSE`.

```{r, eval = FALSE}
testthat::test_that(
  "Invalid dates return HTTP response status != 200.",
  {
    # parameters
    ...
    commands <- read_commands(commands_path = commands_path)
    urls <- extract_urls(commands = commands, position = 6)
    url_status <- check_urls(urls = urls, size = 2, method = "HEAD")
    # test for false
    expect_false(all(url_status))
  }
)
```
...
### Prerequisite files (Mac/Linux)

With a NASA EarthData Account and the required applications authorized to use the credentials, it is time to create the prerequisite files. If working on a **Mac/Linux machine**, open a terminal window.

**Note** As the steps to produce the prerequisite files is very specific, much of the code used is taken verbatim from [How to Generate Earthdata Prerequisite Files](https://disc.gsfc.nasa.gov/information/howto?title=How%20to%20Generate%20Earthdata%20Prerequisite%20Files) on NASA GES DISC's "How-To's" webpage ([link](https://disc.gsfc.nasa.gov/information/howto)).

#### `.netrc`

Type the following commands in order replacing `YOUR_USERNAME` and `YOUR_PASSWORD` with your NASA EarthData username and password, respectively.
...
Type the following commands in order replacing `YOUR_HOME_DIRECTORY` with the absolute path of your home directory.

```{bash, eval = FALSE}
cd ~
touch .dodsrc
echo "HTTP.NETRC=YOUR_HOME_DIRECTORY/.netrc" >> .dodsrc
echo "HTTP.COOKIE.JAR=YOUR_HOME_DIRECTORY/.urs_cookies" >> .dodsrc
```
...
## Prerequisite files (Windows)

With a NASA EarthData Account and the required applications authorized to use the credentials, it is time to create the prerequisite files. The following are steps for users with **Windows machines**.
...
Replace `YOUR_USERNAME` and `YOUR_PASSWORD` with your NASA EarthData username and password, respectively. Save file as `C:\.netrc`.

#### `.urs_cookies`

Open a run command window. Type "cmd" in text field and click "OK".
...
Open a run command window. Type "cmd" in text field and click "OK". Navigate to the root user directory.
...
Replace `YOUR_HOME_DIRECTORY` with the absolute path of your home directory. Copy `.dodsrc` to the working directory.
