...
This test utilizes `expect_message()` because the `data_download()` wrapper function returns a message if the underlying source-specific download function returns an error. If we directly used the `download_noaa_hms_smoke` funciton, we would expect and recieve an error.
