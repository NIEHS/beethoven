...
scriptlines_regular <- glue::glue(
    "#SBATCH --job-name=gen_beethoven \
#SBATCH --ntasks=1 \
...
controller_general_beethoven <- crew.cluster::crew_controller_slurm(
    ...
    options_cluster = crew.cluster::crew_options_slurm(
        ...
        script_lines = scriptlines_regular,
...
scriptlines_download_norm <- glue::glue(
    "#SBATCH --job-name=download \
#SBATCH --ntasks=1 \
#SBATCH --output=slurm/%x_%j.out \
#SBATCH --error=slurm/%x_%j.err \
...
controller_download_norm <- crew.cluster::crew_controller_slurm(
    ...
    options_cluster = crew.cluster::crew_options_slurm(
        verbose = TRUE,
        partition = "normal,highmem,geo",
        script_lines = scriptlines_download_norm,
