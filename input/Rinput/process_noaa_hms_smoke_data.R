...
################################################################################
#' process_noaa_hms_smoke_data: download daily wildfire smoke plume data from
...
#' (raw) data.
#' @param separate_output logical(1). Export separate files for "light",
#' "medium", "heavy" smoke density classification. If `TRUE` one file containing
#' all density classification data is saved.
#' @param output_format character(1). "Shapefile" or "Geopackage". Format to
#' save processed data.
#' @param directory_with_downloaded_data character(1). Directory storing
#' downloaded ".shp" or ".kml" files downloaded from NOAA Hazard Mapping System
#' Fire and Smoke Product.
#' @param directory_to_save character(1). Directory to save processed files.
#' @param remove_downloaded logical(1). Remove downloaded (raw) data files in
#' directory_with_downloaded_data.
#' @author Mitchell Manware.
#' @return NULL; Processed NOAA Hazard Mapping System Fire and Smoke Product
#' data will be returned to the designated saving directory in the indicated
#' format.
#' @export
process_noaa_hms_smoke_data <- function(
  ...
  if (substr(directory_with_downloaded_data,
             chars_dir_download,
             chars_dir_download) != "/") {
    directory_with_downloaded_data <- paste(directory_with_downloaded_data,
                                            "/",
                                            sep = "")
  }
  ...
  if (data_format == "Shapefile") {
    ...
    for (h in seq_along(hms_files_shp)) {
      if (h == 1) {
        ...
        smoke_data <- terra::aggregate(smoke_data,
                                       by = "Density",
                                       dissolve = TRUE)
        smoke_data$Date <- paste0(date_sequence[h])
      } else if (h > 1) {
        smoke_data_2 <- terra::vect(hms_files_shp[h])
        smoke_data_2 <- terra::aggregate(smoke_data_2,
                                         by = "Density",
                                         dissolve = TRUE)
  ...
  #   }
  #   cat(paste0("Data files removed from ",
  #              directory_with_downloaded_data,
  #              ".\n"))
  # }
  #nocov end
