...
download_aqs_data <- function(
  ...
  if (substr(directory_to_download,
             chars_dir_download, chars_dir_download) != "/") {
  ...
  # The unique site identifier "ID.Monitor" is
  # a string with the structure State-County-Site-Parameter-POC
  for (n in seq(1, length(file_urls))) {
    ...
    # ISong: Some POCs are two digits,
    # so here I changed POC slot to zero-padded two digits.
    data$ID.Monitor <-
  ...
  cat(paste("All requested files were downloaded.
