...
download_noaa_hms_smoke_data <- function(
  ...
  for (f in seq_along(date_sequence)) {
    ...
    if (data_format == "Shapefile") {
      file_urls <- paste0(url_noaa_hms_smoke_data,
                          data_format,
                          "/",
                          year,
                          "/",
                          ...
                          date_sequence,
                          ".zip")
      download_names <- paste0(directory_to_download,
                               "hms_smoke_",
                               data_format,
                               "_",
                               date_sequence,
  ...
  if (data_format == "Shapefile") {
    ...
    names_to_change <- list.files(path = directory_to_save,
                                  pattern = "hms_smoke",
                                  full.names = TRUE)
    ...
    cat(paste0("Unzipping KML files to ",
               directory_to_save,
               "...\n"))
    for (u in seq_along(download_names)) {
