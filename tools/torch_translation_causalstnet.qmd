...
STCN(24, 18, 1, c(18, 36, 144, 36, 3), 4, 0.25)
...
xtss <- load_data(as.array(prep_tensor$xtensor), as.array(prep_tensor$ytensor))


torch::torch_device(config$device)
xtsstr <-
train_main(model_name = "STCN", fulldata = lapply(xtss, torch::torch_tensor, device = torch_device(config$device)), dinput = config$input_size, doutput = config$output_size, dhidden = config$hidden_size, nlayers = config$num_layers, nchannels = config$in_channels, nlvs = config$nlevels, dkernel = config$kernel_size, p_dropout = config$dropout)
