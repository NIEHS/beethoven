...
mons_yr_df = mons_yr_df %>%
    ...
    transmute(uniqueid = sprintf("%02d-%03d-%04d-%05d-%02d", `State Code`, `County Code`, `Site Num`, `Parameter Code`, `POC`),
    long = Longitude,
    lat = Latitude,
    datum = Datum,
    year = Year,
...
method_measure_lookup = 
    data.frame(
        ...
        Method = c("Gravimetric", "Beta Attenuation", "Laser Light Scattering", "Broadband Spectroscopy", "Not Specified",
...
mons_yr_df_no_unique = mons_yr_df_no %>%
    ...
    ungroup() %>%
    tidytable::group_by(uniqueid) %>%
    summarize(n_obs_yr = sum(n_obs, na.rm = T),
              n_valid_yr = sum(n_valid, na.rm = T),
              n_req_yr = sum(n_req, na.rm = T),
              n_below_mdl_yr = sum(n_below_mdl, na.rm = T)) %>%
    ungroup() #%>%
...
tbls_pm_start_end_by_duration = 
    ...
    summarize(date_start= min(date_local),
              date_end = max(date_local)) %>%
    ungroup()
...
lookup_pm_uniques =
    ...
    summarize(duration1 = unique(duration)[1],
...
pm_monitor_change_periods = 
    ...
    dplyr::summarize(date_start = min(date_local),
                     date_end = max(date_local)) %>%
    ungroup() %>%
...
generate_fullts = \(vstart, vend, index) length(unique(index)) %>% split(., .) %>% lapply(\(x) seq(vstart[x], vend[x], 1)) 


cat(sprintf("%d monitors have the history of method changes during the study period.\n",
...
tbls_pm_cleaned =
    ...
    as_tibble() %>%
    dplyr::group_by(unique2) %>%
    dplyr::mutate(methods_all = paste(unique(method_measure_samp[which(method_measure_samp != "ns")]), collapse = "|"),
...
mon_state = 
    ...
    dplyr::summarize(Nmonitors = length(unique(unique2)),
              Nsites = length(unique(gsub("-[0-9]+{2,2}$", "", unique2)))) %>%
    ungroup %>%
    dplyr::mutate(state = str_replace(state, "Of ", "of"),
...
lvs_pm = c("Beta Attenuation", "Broadband Spectroscopy", "Gravimetric",
...
tbls_pm_summary = tbls_pm_cleaned_qa %>%
    ...
    tidytable::summarize(date_start = min(date_local),
              date_end = max(date_local),
...
tbls_pm_summary_gg = 
    ggplot(data = tbls_pm_summary) +
        geom_segment(data = tbls_pm_summary, 
                    mapping = aes(x = date_start, xend = date_end_line, y = unique2, yend = unique2, color = method, 
        ...
        guides(color = guide_legend(override.aes = list(linewidth = unit(1, 'cm'))),
               linewidth = "none")
# ggsave(plot = tbls_pm_summary_gg, filename = "bystate.png",
#         width = 24, height = 15, units = "in", dpi = 508)
tbls_pm_summary_gg  + 
  labs(title = bquote(atop("PM"[2.5]~ "measurement patterns by methods","       across 48 mainland states and D.C.")),
...
tbls_pm_frequency = 
    ...
    as_tibble() %>%
    dplyr::group_by(unique2) %>%
    dplyr::summarize(long = unique(long),
              lat = unique(lat),
...
sf_pm_daily_year_rate_site = as_tibble(tbls_pm_daily_year_rate) %>%
    ...
    dplyr::summarize(`Measurement types` = paste(`Measurement types`, collapse = "|"),
...
tbls_pm_nest = as_tibble(tbls_pm_cleaned_qa) %>%
    ...
    dplyr::summarize(siteid = unique(siteid),
...
plot_empty <- plotly_empty() %>% layout(plot_bgcolor = "rgba(0,0,0,0)",
    ...
    lapply(function(x) {
        if (!is.na(x$nullplot)) {
            ...
            tbls_pm_summary %>%
                ...
                add_segments(x = ~date_start, y = ~unique2, xend = ~date_end_line, yend = ~unique2,
                             hovertemplate = "Monitor: %{y}",
                             hoverinfo = 'y') %>%
                layout(yaxis = list(showticklabels = FALSE),
                       xaxis = list(title=""),
                       annotations = list(orientation = "h", align = "left", x=0.5, y = 1.05, 
...
output$p2 = renderPlotly({
    ...
    if (is.null(res_data)) {print("Selected sites data will appear here (drag to select)\n" )}
        ...
        ggt = ggplot(data = target_full,
            ...
            guides(color = guide_legend(override.aes = list(linewidth = unit(0.5, 'cm'))),
               linewidth = "none")
        plotly::ggplotly(ggt) %>%
