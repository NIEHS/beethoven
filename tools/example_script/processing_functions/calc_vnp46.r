...
#   }
test <- FALSE
if (test) year <- 2018; threads <- 50L

# mod09_dir <- file.path(rootdir, userdir, inputdir, "modis", "raw", "5000", "VNP46A2")
...
#              full.names = TRUE,
#              recursive = TRUE)
# writeLines(files_mod09, "~/rtest/vnp46filelist.txt")
...
saveRDS(
  ...
  file.path(
    rootdir,
    "home/songi2",
    sprintf("NRTAP_Covar_VNP46A2_%d.rds", as.integer(year))
  ),
  compress = "xz"
)
# mod09_vars1 <-
