...
test_that("convert_stobj_to_stdt works well", {
  ...
  expect_error(convert_stobj_to_stdt(stobj = "I love cheese"),
  ...
  df$time <- NULL
  expect_error(convert_stobj_to_stdt(df), 
  ...
  expect_equal(stdt[lon == -112 & lat == 35.35 & time == "2023-11-02", var1],
               6)
  expect_equal(stdt[lon == -112 & lat == 35.35 & time == "2023-11-02", var2], 
               56)
  ...
  stobj$tjme <- stobj$time
  stobj$time <- NULL
  expect_error(convert_stobj_to_stdt(stobj), 
  ...
  # 1) it should work
  var1 <- rast(extent = c(-112, -101, 33.5, 40.9), 
               ncol = 5, 
               nrow = 5, 
               crs = "epsg:4326")
  ...
  var2 <- rast(extent = c(-112, -101, 33.5, 40.9), 
               ncol = 5, 
               nrow = 5, 
               crs = "epsg:4326")
